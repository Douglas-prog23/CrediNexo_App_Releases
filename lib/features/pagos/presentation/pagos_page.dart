...
import 'credito_pago_detail_page.dart';
...
class _PagosPageState extends State<PagosPage> {
  ...
  Future<void> _openDetail(CreditoDisponible credito) async {
    await Navigator.of(context).push(
      MaterialPageRoute(
          builder: (_) => CreditoPagoDetailPage(credito: credito)),
    );
    _reload();
  }
                ...
                if (rows.isEmpty)
                  EmptyView(
                    title: _query.isEmpty
                        ? 'Sin creditos para pago'
                        : 'Sin resultados',
                    message: _query.isEmpty
                        ? 'No hay creditos asignados para este usuario.'
                        : 'No encontramos creditos con esa busqueda.',
                  )
                else
                  ...rows.map((credito) => _CreditoCard(
                      credito: credito, onTap: () => _openDetail(credito))),
                const SizedBox(height: 12),
                _PaginationFooter(
                  pageData: pageData,
    ...
    return Card(
      ...
      child: InkWell(
        ...
        child: ListTile(
          ...
          title: Text(
            credito.cliente.nombreCompleto.isEmpty
                ? 'Cliente sin nombre'
